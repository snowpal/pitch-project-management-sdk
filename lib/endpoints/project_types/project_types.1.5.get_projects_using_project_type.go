...
func GetProjectsUsingProjectType(jwtToken string, projectTypeId string) ([]response.Project, error) {
	resProjects := response.Projects{}
	route, err := helpers2.GetRoute(lib.RouteProjectTypesGetProjectsUsingProjectType, projectTypeId)
	if err != nil {
		fmt.Println(err)
